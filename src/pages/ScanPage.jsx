...
import {
  ...
  Droplets, Activity, Layers, Zap, Shield,
...
const SCORE_ITEMS = [
  ...
  { key: 'evenness',  icon: Layers,   label: { vi: 'Đều Màu',     en: 'Evenness'  }, isPositive: true },
  { key: 'clarity',   icon: Shield,   label: { vi: 'Độ Sạch',     en: 'Clarity'   }, isPositive: true },
]
...
const RESULTS_CONTENT = {
  vi: {
    ...
    cards: { skinType: 'Loại Da', concerns: 'Vấn Đề Phát Hiện', mainConcern: 'Mối Quan Tâm Chính', advice: 'Lời Khuyên AI' },
    aiLabel:   'Đánh Giá AI',
    scores: {
      title:         'Điểm Số Chi Tiết',
      highLabel:     'Điểm cao:',
      highText:      'cho thấy làn da khỏe mạnh ở khía cạnh đó.',
      lowLabel:      'Cần chú ý:',
      lowText:       'khía cạnh cần được chú ý cải thiện.',
    },
    priorities: {
      title:    'Mối Quan Tâm Ưu Tiên',
      prefix:   'Ưu tiên',
      copy:     'Được phát hiện từ ảnh — sẽ được xử lý trong sản phẩm gợi ý.',
    },
  ...
  en: {
    ...
    cards: { skinType: 'Skin Type', concerns: 'Issues Detected', mainConcern: 'Main Concern', advice: 'AI Advice' },
    ...
    scores: {
      ...
      lowText:       'Lower score means a stronger visible concern.',
    },
    priorities: {
      title:    'Priority Concerns',
      prefix:   'Priority',
      copy:     'Detected from photos — addressed in the recommended products.',
    },
...
function getRecommendedProducts(scores = {}, detectedConcerns = []) {
  ...
  // Dark spot serum if uneven tone or dark spots detected
  if ((scores.evenness ?? 100) < 65 || hasConcern('dark spots', 'hyperpigmentation', 'melasma', 'uneven', 'spots'))
    list.push(EUCERIN_PRODUCTS.find(p => p.id === 'anti-pigment-dual-serum'))
  // SPF if low radiance or dark spots
  if ((scores.radiance ?? 100) < 65 || hasConcern('dark spots', 'dull', 'aging'))
    list.push(EUCERIN_PRODUCTS.find(p => p.id === 'sun-fluid-anti-age'))
  // Day cream if low hydration
  if ((scores.hydration ?? 100) < 60)
...
function ResultsStep({ skinType, aiAnalysis, lang, onContinue }) {
  ...
  const mainConcern = concerns[0] || (lang === 'vi' ? 'Chưa xác định' : 'Not detected')
        ...
        <motion.div {...slideUp} className="w-full max-w-4xl space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-2xl bg-gradient-to-br from-primary to-primary/80 p-4 text-center text-white shadow-lg">
              <p className="mb-1 text-xs font-bold uppercase tracking-wider opacity-80">{t.cards.skinType}</p>
              <p className="text-xl font-bold capitalize">{skinType || 'normal'}</p>
            </div>
            <div className="rounded-2xl bg-card p-4 text-center shadow-lg">
              <p className="mb-1 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t.cards.concerns}</p>
              <p className="text-4xl font-bold text-primary">{concerns.length}</p>
            </div>
            <div className="rounded-2xl bg-card p-4 text-center shadow-lg">
              <p className="mb-1 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t.cards.mainConcern}</p>
              <p className="text-sm font-bold text-foreground capitalize leading-snug">{mainConcern}</p>
            </div>
            <div className="rounded-2xl bg-card p-4 text-center shadow-lg">
              <p className="mb-1 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t.cards.advice}</p>
              <p className="text-xs text-foreground leading-snug line-clamp-3">{aiAnalysis?.advice || '—'}</p>
            </div>
          </div>

          {/* Main result headline card */}
          <div className="rounded-2xl border-0 border-l-4 border-l-primary bg-card shadow-lg" style={{ borderLeftWidth: '4px', borderLeftColor: 'oklch(0.4 0.15 15)' }}>
            <div className="p-6">
              <p className="mb-2 text-xs font-bold uppercase tracking-widest text-primary">{t.aiLabel}</p>
              <h2 className="mb-3 text-lg font-bold text-foreground">{aiAnalysis?.headline || skinType}</h2>
              {aiAnalysis?.description && (
                <p className="mb-4 text-sm leading-relaxed text-foreground">{aiAnalysis.description}</p>
              )}
              ...
              {concerns.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {concerns.map(c => (
                    <span key={c} className="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary">{c}</span>
                  ))}
                </div>
              )}
            </div>
                  ...
                  >
                    <div className="p-5">
                      <div className="mb-3 flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-primary">{t.priorities.prefix} {i + 1}</span>
                      </div>
                      <h4 className="mb-2 font-bold capitalize text-foreground">{concern}</h4>
                      <p className="text-sm leading-relaxed text-muted-foreground">{t.priorities.copy}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
